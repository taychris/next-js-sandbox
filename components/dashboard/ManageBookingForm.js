...
const ManageBookingForm = ({ formInformation, appointment }) => {
  const dateNow = format(new Date(), "yyyy-MM-dd'T'HH:mm")
  const [state, setState ] = useState()

  useEffect(() => {
    setState(appointment)
    document.getElementById('startDatetime').value = appointment.startDatetime
    document.getElementById('endDatetime').value = appointment.endDatetime
  }, [])
  ...
  return (
    <div className="max-w-md px-4 mx-auto mb-10 sm:px-7 md:max-w-4xl md:px-6 flex items-center justify-center flex-col">
        <h2 className="mt-10 mb-5 text-left w-full md:w-1/2 font-semibold text-gray-900">Manage the selected appointment.</h2>
